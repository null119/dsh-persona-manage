 ...
 *   PUT  /        → validate + persist + apply { persona }
 *   POST /reset   → clear the persona (restore the composition default)
 *
...
async function writePatchLayer(ctx, persona) {
	...
	if (hasOtherEntries) {
		...
		await writeFile(path, `${withoutMarker.join('\n').trimEnd()}\n${block.join('\n')}\n`, 'utf8')
	}
}
 ...
 * ends with a newline. Returns null when no top-level persona entry exists.
 */
export function replacePresetPersona(text, persona) {
	...
	const row = persona.length === 0
		? ['- id: persona', "  name: '@deepseek-ai/dsh-persona'", '  config:', "    text: ''"]
		: ['- id: persona', "  name: '@deepseek-ai/dsh-persona'", '  config:', '    text: |-', ...blockScalarLines(persona)]
...
/**
 * Rewrite the managed preset's persona row. Every session on the `standard`
 ...
 * by the include watcher), then kick the live row immediately so the change
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			try {
				...
				// POST /reset — restore the composition default (empty persona)
				if (sub === 'reset' && req.method === 'POST') {
					await saveStore('')
					await applyPersona(ctx, '')
					ctx.logger.info('dsh-persona-manage: persona reset to default')
					json(res, { ok: true, ...(await view(ctx)) })
					return
				}
