...
/**
 * Snapshot view, or an explicit unavailable marker. Pass `settleExpect` right
 * after a write to re-sample the live probe through the fiber-restart window
 * (see settleProbe); without it one fast sample serves a plain GET.
 */
async function view(ctx, settleExpect) {
	const live = livePersona(ctx)
	if (live === undefined) {
		return { available: false, error: `composition row "${SYSTEM_PROMPT_ENTRY}" not found in the loader tree` }
	}
	const expect = settleExpect !== undefined ? settleExpect : live.persona
	let probe
	let converged
		...
		handler: async (req, res) => {
			try {
				...
				if (sub === '' && req.method === 'PUT') {
					...
					json(res, { ok: true, ...(await view(ctx, persona)) })
					return
				}
