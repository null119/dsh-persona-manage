 ...
 * (`system-prompt: {persona|personaPrefix/personaSuffix: ''}`, carried in
 * both key generations so any DSH version parses it) written at the first
 * boot after install
 ...
 * registry's own persona sections (`deployment:persona`, or
 * `deployment:persona-prefix`/`-suffix` after the split — a same-name global
 * registration would collide); anchored at the persona prefix's order, so the
 * text renders exactly where the deployment persona belongs — right after
 * the harness identity.
 */
const PERSONA_SECTION_NAME = 'deployment:persona-manage'

/**
 * Resolve the section's sort order across DSH generations. The current
 * harness split the single `DEPLOYMENT_PERSONA` slot into
 * `DEPLOYMENT_PERSONA_PREFIX` (and -SUFFIX); older harness only knows the
 * unsplit key — and `section()` rejects a non-finite order in BOTH, so a
 * plugin hardcoding either key alone crashes the whole plugin tree on the
 * other generation (a distributed plugin must not take its users' DSH
 * down). Probe the split key first (it keeps the historical slot value),
 * fall back to the unsplit key, and finally to the slot's stable value 0:
 * an unknown future table can then only misplace the section, never abort
 * the boot.
 */
function personaSectionOrder(systemPrompt) {
 ...
 * registry's deployment persona section (either key generation) stops
 * rendering the factory text next to this plugin's runtime section. All
 * three keys are pinned at once — schemastery ACCEPTS foreign keys and
 * leaves them unread, so each generation validates the block and honors
 * only the keys its own code consumes: old (`persona`) and new
 * (`personaPrefix`/`personaSuffix`) harness alike (verified against both
 * published schemas).
 * The pin's content NEVER changes with the persona — that is the point:
 * once present, re-applying it is a byte-identical no-op, so persona edits
 * never touch this file and never trigger the include-watcher
 * recomposition (whose reconcile wave wedged the web client's
 * active-session composer).
 */
function managedBlock() {
	return [PATCH_BEGIN, '- id: system-prompt', '  config:', "    persona: ''", "    personaPrefix: ''", "    personaSuffix: ''", PATCH_END]
}
...
/**
 * The preset's persona row, carried in BOTH config generations at once:
 * `prefix` for the split-key dsh-persona and `text` for the unsplit one.
 * Schemastery accepts the foreign key and leaves it unread, so the row
 * loads on old and new DSH alike — and the split-key schema marks
 * `prefix` REQUIRED, so a `text`-only row would fail its validation
 * ("$.prefix missing required value", reproduced against 0.1.5-rc.3).
 */
function presetPersonaRow(persona) {
	return ['- id: persona', "  name: '@deepseek-ai/dsh-persona'", '  config:', '    prefix: |-', ...blockScalarLines(persona), '    text: |-', ...blockScalarLines(persona)]
}
...
export function replacePresetPersona(text, persona) {
	...
	if (start === -1) {
		...
		const row = presetPersonaRow(persona)
...
function livePersona(ctx) {
	...
	// Split-key harness consumes personaPrefix; the unsplit one consumes
	// persona. Foreign keys ride along in the config unread, so prefer the
	// split key and fall back to the unsplit one — when both are present
	// (a pinned row), personaPrefix is the pinned empty one, which is the
	// correct read.
	const raw = config?.personaPrefix ?? config?.persona
...
export async function apply(ctx) {
	...
	// The registration itself is guarded: this plugin ships to users on
	// mixed DSH generations, and an unresolvable anchor (or any future
	// section() validation change) must degrade to "persona not rendered",
	// never abort the host's plugin tree and take the whole DSH down.
	try {
		ctx.effect(() => ctx.systemPrompt.section({
			name: PERSONA_SECTION_NAME,
			order: personaSectionOrder(ctx.systemPrompt),
			text: () => runtime.text,
		}), 'persona-manage: persona section')
	} catch (error) {
		ctx.logger.warn(`dsh-persona-manage: persona section unavailable (${String(error)}) — management routes stay up, persona rendering is skipped`)
	}
