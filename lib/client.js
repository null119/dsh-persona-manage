		...
		*/
		...
		const save = react.useCallback(async (force) => {
			...
			try {
				...
				if (parsed.savedDefault === true) {
					// Empty save → composition default restored. The live row
					// still carries the previous persona until the include
					// watcher recomposes, so keep the editor showing what the
					// user saved (empty) rather than refilling the old text.
					setDraft('')
					setSaved('')
					setLiveInfo(parsed.live)
					setNotice('内容为空 —— 已保存为默认（恢复出厂 persona，新会话生效）')
					return
				}
				setDraft(parsed.persona)
				setSaved(parsed.persona)
				setLiveInfo(parsed.live)
				setNotice(parsed.live !== undefined && parsed.live.matchesRow === false
					? '已保存，但活注册表未同步（见下方诊断）'
					: `已保存（${String(parsed.stats.bytes)} 字节，约 ${String(parsed.stats.estimatedTokens)} token）——热生效，新会话即用新 persona`)
			} catch (err) {
				setError(String(err.message ?? err))
			} finally {
				setBusy(false)
		...
		const reset = react.useCallback(async () => {
			...
			try {
				...
				setSaved('')
				setLiveInfo(parsed.live)
				setNotice('已重置 —— 移除自定义 persona，恢复出厂默认文案（新会话生效）')
			} catch (err) {
				setError(String(err.message ?? err))
			} finally {
				setBusy(false)
			...
			return react.createElement('div', { className: 'pmn-page' },
				react.createElement('div', { className: 'pmn-section' },
					react.createElement('h3', { className: 'pmn-section-head' }, 'Persona 提示词（部署级系统提示词）'),
				...
				liveInfo !== undefined ? react.createElement('div', { className: 'pmn-meta' },
					`活注册表：${liveInfo.serviceAvailable === false ? 'systemPrompt 服务不可见' : liveInfo.sectionFound === false ? '未找到 deployment:persona 段' + (liveInfo.error !== undefined ? `（${liveInfo.error}）` : '') : liveInfo.matchesRow === true ? '✓ 已同步（渲染文本与保存一致）' : `✗ 未同步 —— 段首 80 字符：${liveInfo.preview !== undefined ? JSON.stringify(liveInfo.preview) : '(空)'}`}`) : null,
			))
		}
