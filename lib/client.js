		...
		function PersonaSection(_props) {
			...
			const reset = react.useCallback(async () => {
				...
				try {
					...
					setDraft(parsed.persona)
					setSaved(parsed.persona)
					setNotice('已重置为默认（空 persona，段落在渲染时消失）')
				} catch (err) {
					setError(String(err.message ?? err))
				} finally {
					setBusy(false)
			...
			return react.createElement('div', { className: 'pmn-page' },
				react.createElement('div', { className: 'pmn-section' },
					...
					react.createElement('p', { className: 'pmn-desc' },
						'此文本渲染为系统提示词 order 0 的 deployment:persona 段，紧跟 harness 身份之后、所有工具引导之前，对每个会话生效。保存即写入 settings.yaml 并热应用（无需重启宿主）。'),
					react.createElement('p', { className: 'pmn-desc' },
				...
				react.createElement('div', { className: 'pmn-section' },
					react.createElement('textarea', {
						...
						value: draft,
						spellCheck: false,
						placeholder: '输入 persona 提示词…（留空 = 不渲染该段）',
					...
					react.createElement('div', { className: 'pmn-stats' },
						...
						dirty ? react.createElement('span', { className: 'pmn-err' }, '● 未保存的修改') : null),
