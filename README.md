...
| 面 | 位置 | 作用 |
|---|---|---|
| 持久补丁层 | profile 自己的 `cordis.patch.yml`（由 `ctx.baseUrl` 解析） | **最后**合成层，覆盖 dsh-web-app 自带的 `system-prompt` persona 行；include watcher 热重载。清空时剥离托管块并保留 `[]` 占位符——纯注释文档会解析为 null 并炸掉整个 profile 加载（`must be a top-level YAML array`） |
| 会话层 | `~/.dsh/.agent-presets/re-standard/agent.cordis.yml` 的 `- id: persona` 行 | standard 家族的会话渲染 preset 的 scoped persona（遮蔽部署默认值），**这才是实际到达模型的那份**。非空时按行跨度整行写入（收敛 `''`、`|`/`|-` 块、手工改坏的历史形状；行缺失时自动插回）；**空 persona 时整行移除**——写 `text: ''` 会注册一个空 section 依旧遮蔽，会话将拿不到任何 persona（连出厂默认也没有）。每会话创建时重查，改完新会话即生效。preset 不存在时仅告警 |
| 活树 | `loader` entry `entry.update({ config })` | cordis 将配置 diff 解析为活 fiber 重载；探测发现活注册表未同步时（重启被否决 / 竞态）强制 `entry.fiber.update(config, false)` 兜底。**仅非空 persona 触碰活树**——空值 `entry.update` 会在运行期抹掉行内的出厂默认文案，并重启 system-prompt fiber、级联重载所有注入其服务的插件 |
| store + enforce | `$DSH_HOME/persona-manage/persona.json`（`$DSH_HOME` 未设则 `~/.dsh`） | loader-tree 变更是运行时态（用户补丁 HMR 重载会重置行），故在 `loader/entry-init` / `loader/config-update` 事件上幂等重施加（对比活值，已一致则零副作用）。**空 store 只收敛文件层**（剥离托管块、移除预设行，自愈历史版本写入的空 persona 残留），绝不触碰活树 |
...
- `re-standard` preset 不存在时仅告警（`preset "re-standard" composition not
  found`），standard 家族会话保留各自的 persona，其余各面照常写入。
...
- **Reset / reload / discard**: reset drops the managed override (patch block
  + preset row) so dsh-web-app's factory persona renders again (saving empty
  content is equivalent); also drop the local draft, or re-fetch the server
  snapshot.
...
exists in the composition row's config — so a save writes several surfaces at
once:

| Surface | Location | Role |
|---|---|---|
| Durable patch layer | the profile's own `cordis.patch.yml` (resolved from `ctx.baseUrl`) | The **last** composition layer, overriding dsh-web-app's own `system-prompt` persona row; hot-reloaded by the include watcher. When cleared, the managed block is stripped and the `[]` placeholder preserved — a comments-only document parses as null and fails the whole profile load |
| Session layer | the `- id: persona` row of `~/.dsh/.agent-presets/re-standard/agent.cordis.yml` | Sessions on the `standard` family render the preset's scoped persona, which shadows the deployment default — **this is the text that actually reaches the model**. Non-empty personas are written as a whole line-span (every historical shape converges; the row is re-inserted when absent); an **empty persona removes the row entirely** — writing `text: ''` would register an empty section that still shadows, leaving sessions with no persona at all (not even the factory default). Re-checked per session creation. Absent preset ⇒ warning only |
| Live tree | the loader entry via `entry.update({ config })` | cordis reconciles into a live fiber reload; if the probe shows the registry did not pick the change up, `entry.fiber.update(config, false)` forces it. **Only a non-empty persona ever touches the live tree** — an empty-value update would blank the row's factory text at runtime and restart the system-prompt fiber, cascading a reload through every plugin injecting its services |
| Store + enforce | `$DSH_HOME/persona-manage/persona.json` (`~/.dsh` when `$DSH_HOME` is unset) | Loader-tree mutations are runtime-only (a user-patch HMR reload resets rows), so the persona is idempotently re-enforced on `loader/entry-init` and `loader/config-update`. **An empty store only converges the file layers** (strips the managed block, removes the preset row — self-healing residue older versions wrote), never touching the live tree |
...
- A missing `re-standard` preset only warns; sessions on the standard preset
  keep their own persona while the other surfaces are written as usual.
